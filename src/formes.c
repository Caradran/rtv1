...
/*   Updated: 2018/02/10 09:33:25 by esuits           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_formes		*init_formes(t_env *env)
{
	...
	formes->next->next->next->sph = init_sph(init_vect(0.2,0,0), 0.1, init_col(1,1,1,1));
	if (!(formes->next->next->next->next = malloc(sizeof(t_formes))))
		return (NULL);
	formes->next->next->next->next->type = 2;
	formes->next->next->next->next->plan = init_plan(init_vect(1, 0, 0), 1, init_col(1, 1, 1, 1));
...
//	lights->next = NULL;
	if (!(lights->next = malloc(sizeof(t_lights))))
		return (NULL);
	lights->next->lgt = init_lgt(init_col(0, 1, 0, 1), init_vect(0, n*0.5, n*0.866));
