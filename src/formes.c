...
/*   Updated: 2018/01/31 00:11:58 by esuits           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_formes		*init_formes(t_env *env)
{
	...
	if (!(formes = malloc(sizeof(t_formes))))
		return (NULL);
	formes->type = 1;
	formes->sph = init_sph(init_vect(1, 0, 0), 0.2, init_col(1, 0.5, 0.5, 1));
	...
	formes->next->next->plan = init_plan(init_vect(0.1, 0, 1), 0.5,
			init_col(0, 1, 0, 1));
...
t_lights		*init_lights(t_env *env)
{
	...
	if (!(lights = malloc(sizeof(t_lights))))
		return (NULL);
	lights->lgt = init_lgt(init_col(1, 1, 1, 1), init_vect(1000, 100, 0));
	lights->next = NULL;
	return (lights);
}
